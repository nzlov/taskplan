  ...
  export default {
    ...
    methods: {
      ...
      save() {
        ...
        if (this.editedIndex > -1) {
          if (this.$refs.form.validate()) {
            this.commit = true;
            this.$http.LPost(this.$store.state, `/usergroup/${this.editedItem.id}`, this.editedItem).then((resp) => {
              this.commit = false;
          ...
          if (this.$refs.form.validate()) {
            this.commit = true;
            this.$http.LPost(this.$store.state, '/usergroup', this.editedItem).then((resp) => {
              this.commit = false;
              switch (resp.data.code) {
                case 0: {
                  this.updateData();
      ...
      del() {
        this.alert_error = false;
        this.commit = true;
        this.$http.LDel(this.$store.state, `/usergroup/${this.delItem.id}`).then((resp) => {
          this.commit = false;
          switch (resp.data.code) {
            case 0: {
              this.updateData();
      ...
      getDataFromApi() {
        ...
        return new Promise((resolve) => {
          ...
          this.$http.LGet(this.$store.state, `/usergroup?${order}&offset=${offset}&limit=${rowsPerPage}${search}`).then((resp) => {
            this.loading = false;
            switch (resp.data.code) {
              case 0: {
